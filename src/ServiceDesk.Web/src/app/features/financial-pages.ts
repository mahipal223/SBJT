...
export class InvoiceDetailLivePage {
  ...
  private readonly id=this.route.snapshot.paramMap.get('id')??'';
  constructor(){this.load();}
  load(){this.loading.set(true);this.api.invoice(this.id).subscribe({next:value=>{this.invoice.set(value);this.loading.set(false);},error:error=>{this.error.set(financialMessage(error));this.loading.set(false);}});}
  ...
  pay(){if(!this.invoice())return;this.saving.set(true);this.error.set('');this.api.recordPayment(this.id,this.invoice()!.balance).subscribe({next:()=>{this.message.set('Payment recorded successfully.');this.saving.set(false);this.load();},error:error=>{this.error.set(financialMessage(error));this.saving.set(false);}});}
  printDocument(){window.print();}
