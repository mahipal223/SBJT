    ...
    <main class="page">
      ...
      <section class="card section-gap">
        ...
        @else {<div class="table-scroll"><table class="data-table"><thead><tr><th>Invoice</th><th>Customer</th><th>Issued / due</th><th>Status</th><th>Delivery</th><th>Total</th><th>Balance</th><th></th></tr></thead><tbody>
          @for(i of invoices();track i.id){<tr><td><span class="cell-main"><a [routerLink]="['/app/invoices',i.id]"><strong>#{{i.invoiceNumber}}</strong></a><small>{{i.items.length}} line items</small></span></td><td>{{i.customerName}}</td><td><span class="cell-main"><strong>{{i.issuedOn?(i.issuedOn|date:'MMM d, y'):'Not issued'}}</strong><small>{{i.dueOn?'Due '+(i.dueOn|date:'MMM d, y'):'Draft'}}</small></span></td><td><span class="badge" [class.gray]="i.status==='Draft'" [class.red]="i.isOverdue">{{i.isOverdue?'Overdue':i.paymentStatus==='Paid'?'Paid':i.status}}</span></td><td>@if(i.status!=='Draft'){<span class="badge" [class.gray]="i.deliveryStatus==='NotSent'" [class.amber]="i.deliveryStatus==='Queued'" [class.red]="i.deliveryStatus==='Failed'">{{deliveryLabel(i.deliveryStatus)}}</span>}@else{<span class="badge gray">Draft</span>}</td><td class="money">{{i.total|currency}}</td><td class="money">{{i.balance|currency}}</td><td><div class="page-actions" style="flex-wrap: nowrap; justify-content: flex-end;">@if(i.status==='Draft'){<button class="btn small primary" [disabled]="savingId()===i.id" (click)="issue(i)">Issue</button>}@else if(i.status==='Issued'&&i.balance>0){<button class="btn small" [disabled]="savingId()===i.id" (click)="pay(i)">Record paid</button>}</div></td></tr>}
        </tbody></table></div>}
      </section>
    </main>`
})
export class InvoicesLivePage {
  ...
  pay(invoice: Invoice) { this.savingId.set(invoice.id); this.error.set(''); this.api.recordPayment(invoice.id, invoice.balance).subscribe({next: () => this.load(), error: error => {this.error.set(financialMessage(error)); this.savingId.set('');}}); }
...
@Component({
  ...
  template: `<main class="page"><header class="page-head"><div><nav class="breadcrumb"><a routerLink="/app/estimates">Estimates</a><span class="crumb-sep">/</span><span class="crumb-current">#{{estimate()?.estimateNumber}}</span></nav><div class="title-with-badge"><h1>#{{estimate()?.estimateNumber}} · revision {{estimate()?.revision}}</h1>@if(estimate();as e){<span class="badge" [class.gray]="e.status==='Draft'" [class.amber]="e.status==='Sent'">{{e.status}}</span>}</div><p>{{estimate()?.customerName}} · <a class="link" [routerLink]="['/app/jobs', estimate()?.jobId]">View job</a></p></div><div class="page-actions"><button class="btn" (click)="downloadPdf()"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg> Download PDF</button><button class="btn" (click)="printDocument()"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9V2h12v7M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><path d="M6 14h12v8H6z"/></svg> Print</button>@if(estimate()?.status==='Draft'){<button class="btn primary" [disabled]="saving()" (click)="send()">Send estimate</button>}@if(estimate()&&estimate()!.status!=='Superseded'){<button class="btn" [disabled]="saving()" (click)="revise()">Create revision</button>}@if(estimate()?.status==='Sent'){<button class="btn" [disabled]="saving()" (click)="createLink()">Customer link</button>}</div></header>
